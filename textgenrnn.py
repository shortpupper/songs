...
#rom tensorflow.keras.utils import multi_gpu_model
...
class textgenrnn:
    ...
    def train_new_model_pure(self, texts, context_labels=None, num_epochs=50,
                        gen_epochs=1, batch_size=128, dropout=0.0,
                        train_size=1.0,
                        validation=True, save_epochs=0,
                        multi_gpu=False, **kwargs):
        ...
        self.train_on_texts_pure(texts, new_model=True,
                            via_new_model=True,
                            context_labels=context_labels,
                            num_epochs=num_epochs,
                            gen_epochs=gen_epochs,
...
#edit
    def train_from_largetext_file(self, file_path, new_model=True, save_drive=False, **kwargs):
        ...
        if new_model:
            ...
            self.train_on_texts(texts, save_drive, single_text=True,  **kwargs)
    
    def train_from_largetext_file_pure(self, file_path, new_model=True, save_drive=False, **kwargs):
        with open(file_path, 'r', encoding='utf8', errors='ignore') as f:
            texts = [f.read()]

        if new_model:
            self.train_new_model_pure(
                texts, single_text=True, **kwargs)
        else:#eit
            self.train_on_texts_pure(texts, save_drive, single_text=True,  **kwargs)
